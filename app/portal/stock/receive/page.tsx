...
export default function Page() {
  ...
  const load = async () => {
    ...
    try {
      const [pRes, lRes, rRes] = await Promise.all([
        ...
        fetch("/api/stock/receive", { cache: "no-store" })
      ]);

      const [pData, lData, rData] = await Promise.all([pRes.json(), lRes.json(), rRes.json()]);

      if (!pRes.ok || !lRes.ok || !rRes.ok) {
        setError(pData?.error ?? lData?.error ?? rData?.error ?? "ไม่สามารถโหลดข้อมูลได้");
        return;
      }
  ...
  const createReceive = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setSnack({ open: true, message: data?.error ?? "บันทึกรายการรับเข้าไม่สำเร็จ", severity: "error" });
        return;
      }

      setSnack({ open: true, message: "บันทึกรายการรับเข้าเรียบร้อย", severity: "success" });
  ...
  const startEdit = (row: ReceiveRow) => {
    setEditingId(row.id);
  ...
  const saveEdit = async () => {
    ...
    setSavingEdit(true);
    try {
      const res = await fetch(`/api/stock/receive/${editingId}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
  ...
  const deleteRow = async (id: string) => {
    const res = await fetch(`/api/stock/receive/${id}`, { method: "DELETE" });
    const data = await res.json();
    if (!res.ok) {
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="h5" fontWeight={700}>รับเข้าสินค้า</Typography>
      ...
      <Card elevation={0} sx={{ border: "1px solid #e5e7eb", borderRadius: 3 }}>
        <CardContent>
          <Stack spacing={1.2}>
            ...
            <Button variant="contained" onClick={createReceive} disabled={savingCreate || loading}>
              {savingCreate ? "กำลังบันทึก..." : "บันทึกรายการรับเข้า"}
            </Button>
          </Stack>
        </CardContent>
      </Card>
            ...
            <CardContent>
              <Stack spacing={1}>
                <Stack direction="row" justifyContent="space-between">
                  ...
                  <Chip size="small" color="success" label="รับเข้า" />
                </Stack>
                <Typography variant="body2" color="text.secondary">{dayjs(r.movement_date).format("DD/MM/YYYY HH:mm")}</Typography>
                <Typography variant="body2">จำนวน: {Number(r.qty_in).toLocaleString()}</Typography>
                {editingId === r.id ? (
                  <>
          ...
          <TableBody>
            {pagedRows.map((r) => (
              <TableRow key={r.id} hover>
                ...
                <TableCell align="right">{Number(r.qty_in).toLocaleString()}</TableCell>
                <TableCell width={180}>
